...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor, Settings* settings, const string &_nameSeq):
    ...
    mbUseIMUFallback(true), mbTrackingWithIMU(false), mfIMUFallbackThreshold(0.1),
    mTimeLastGoodPose(0.0)
{
    // IMU Fallback 초기화
    // mbUseIMUFallback = true;  // 기본적으로 활성화
    // mbTrackingWithIMU = false;
    // mfIMUFallbackThreshold = 0.1;  // 적절한 임계값 설정
    // mTimeLastGoodPose = 0.0;
    mLastGoodTcw = cv::Mat::eye(4, 4, CV_32F);
    mIMUVelocity = cv::Mat::zeros(3, 1, CV_32F);
    mLastAngularVelocity = Eigen::Vector3f(0, 0, 0);
...
// added by ejlee 0304
bool Tracking::PredictIMUOnlyPose()
{
    if(!mLastFrame.isSet() || !mCurrentFrame.mpImuPreintegrated)
        return false;
    
    // IMU 데이터만 사용해 간단하게 포즈 업데이트
    ...
    // IMU로 위치 예측 - 바이어스 정보 추가
    IMU::Bias lastBias = mLastFrame.mImuBias; // 또는 mCurrentFrame.mImuBias 사용
    
    Eigen::Matrix3f Rwb2 = Rwb1 * mCurrentFrame.mpImuPreintegrated->GetDeltaRotation(lastBias);
    Eigen::Vector3f twb2 = twb1 + Vwb1*t12 + 0.5f*t12*t12*Gz + Rwb1*mCurrentFrame.mpImuPreintegrated->GetDeltaPosition(lastBias);
    Eigen::Vector3f Vwb2 = Vwb1 + Gz*t12 + Rwb1*mCurrentFrame.mpImuPreintegrated->GetDeltaVelocity(lastBias);
    ...
    Verbose::PrintMess("Using only IMU for pose prediction", Verbose::VERBOSITY_NORMAL);
    return true;
}

//added by ejlee 0304
bool Tracking::ContinueTrackingWithIMU()
{
    if(!mLastFrame.isSet() || !mCurrentFrame.mpImuPreintegrated)
    {
        return false;
    ...
    Verbose::PrintMess("Attempting to continue tracking with IMU in LOST state", Verbose::VERBOSITY_NORMAL);
    ...
    Verbose::PrintMess("IMU-based tracking found: " + to_string(nInliers) + " inliers", Verbose::VERBOSITY_NORMAL);
    ...
    if(nInliers > 10)
    {
        ...
        if(nInliers >= 10)
        {
            mState = OK;  // 충분한 인라이어가 있으면 OK 상태로 변경
                // 다음 라인 추가
            if(!mlbLost.empty())
                mlbLost.back() = false;  // 현재 프레임을 lost가 아니라고 표시
            return true;
        }
    }
    
    // 인라이어가 부족하면 RECENTLY_LOST 상태로 설정하고 계속 트래킹 시도
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        else
        {
            ...
            // if(mState==LOST)
            if(mState==RECENTLY_LOST || mState==LOST)//&& !bOK && mCurrentFrame.mTimeStamp-mTimeStampLost>time_recently_lost_visual)
            {
                if(mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
        ...
        if(mState==LOST)
        {
            ...
            if(mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
            {
                // 수정: IMU 초기화 여부와 상관없이 IMU를 사용하여 트래킹 유지 시도
                if(ContinueTrackingWithIMU())
                {
                    return;
                }
                else if(!pCurrentMap->isImuInitialized())
                {
                    // IMU 초기화되지 않았을 때 특별히 처리
                    Verbose::PrintMess("IMU not initialized, try using it anyway for tracking...", Verbose::VERBOSITY_NORMAL);
                    
                    // IMU만을 사용한 간단한 포즈 예측
                    if(PredictIMUOnlyPose())
                    {
                        mState = RECENTLY_LOST;  // 트래킹 상태를 RECENTLY_LOST로 변경해 복구 기회 제공
                        return;
                    }
                }
            }
            ...
            // 현재 맵이 충분한 키프레임을 포함하고 있는지 확인
            Map* pCurrentMap = mpAtlas->GetCurrentMap();
            if(pCurrentMap->KeyFramesInMap()<=1)
            {
                cout << "Tracking lost but saving current map..." << endl;
            
                mpSystem->SaveTrajectoryEuRoC("map/lost");
                mpSystem->SaveKeyFrameTrajectoryEuRoC("map/lost");
                mpSystem->ResetActiveMap();
                return;

                // if(!mpSystem->mStrSaveAtlasToFile.empty())
                // {
                //     // 백업 저장 로직 - SaveAtlas 함수 직접 호출이 어려울 수 있음
                //     // 필요시 System 클래스에 BackupAtlas 같은 함수 추가 고려
...
{
    // ejlee added 0304 IMU 모드일 때는 리셋 전에 IMU 트래킹 시도
    if(mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
    {
        Verbose::PrintMess("Reset requested, trying IMU tracking first", Verbose::VERBOSITY_NORMAL);
        
        // IMU 초기화와 상관없이 IMU 트래킹 시도
        if(ContinueTrackingWithIMU() || PredictIMUOnlyPose())
        {
            // IMU 트래킹이 성공하면 맵 리셋하지 않음
            return;
        }
    }
