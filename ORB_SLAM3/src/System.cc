...
void System::Shutdown()
{
    ...
    if(!mStrSaveAtlasToFile.empty())
    {
        ...
        SaveTrajectoryEuRoC(mStrSaveAtlasToFile);
        SaveKeyFrameTrajectoryEuRoC(mStrSaveAtlasToFile);
    }
...
void System::SaveTrajectoryEuRoC(const string &filename)
{
    ...
    string nameTrajectoryEuRoC = "./" + filename + "_trajectory";
    /*if(mSensor==MONOCULAR)
    {
    ...
    for(auto lbL = mpTracker->mlbLost.begin(); lbL != mpTracker->mlbLost.end(); lbL++) {
        if(!*lbL) {
            hasValidPose = true;
            break;
        }
    ...
    if(!hasValidPose) {
        cout << "경고: 유효한 포즈가 없습니다. 모든 프레임 포함해서 저장합니다." << endl;
        // 모든 포즈 저장 로직...
    }
...
{
    cout << endl << "Saving keyframe trajectory to " << filename << " ..." << endl;
    string nameKeyFrameTrajectoryEuRoC = "./" + filename + "_KeyFrameTrajectoryEuRoC";
    vector<Map*> vpMaps = mpAtlas->GetAllMaps();
    Map* pBiggerMap;
    int numMaxKFs = 0;
    for(Map* pMap :vpMaps)
    ...
    // After a loop closure the first keyframe might not be at the origin.
    ofstream f;
    f.open(nameKeyFrameTrajectoryEuRoC.c_str());
    f << fixed;
